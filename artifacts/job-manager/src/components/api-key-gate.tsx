...
import { ApiError, getJobSummary, setAuthTokenGetter } from '@workspace/jobs-api-client-react';
...
// Every request now needs a Bearer key (see jobsApiAuth); the UI prompts for
...
async function verifyKey(key: string): Promise<{ ok: true } | { ok: false; message: string }> {
  try {
    await getJobSummary({ headers: { Authorization: `Bearer ${key}` } });
    return { ok: true };
  } catch (err) {
    if (err instanceof ApiError && err.status === 401) {
      return { ok: false, message: 'Invalid API key.' };
    }
    return { ok: false, message: 'Could not reach the Job Manager API. Try again.' };
  }
}
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center p-6">
      <Card className="w-full max-w-sm">
        <CardHeader>
          <CardTitle>Job Manager — API Key Required</CardTitle>
        </CardHeader>
        <CardContent>
