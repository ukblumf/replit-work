...
import { ApiError, getStockSummary, setAuthTokenGetter } from '@workspace/api-client-react';
...
async function verifyKey(key: string): Promise<{ ok: true } | { ok: false; message: string }> {
  try {
    await getStockSummary({ headers: { Authorization: `Bearer ${key}` } });
    return { ok: true };
  } catch (err) {
    if (err instanceof ApiError && err.status === 401) {
      return { ok: false, message: 'Invalid API key.' };
    }
    return { ok: false, message: 'Could not reach the Stock Control API. Try again.' };
  }
}
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center p-6">
      <Card className="w-full max-w-sm">
        <CardHeader>
          <CardTitle className="uppercase">Stock Control — API Key Required</CardTitle>
        </CardHeader>
        <CardContent>
